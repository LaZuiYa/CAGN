...
def parse_args():
    ...
    parser.add_argument('--num_filter', type=int, default=384, help='num of filters')
    ...
    parser.add_argument('--diffusion', type=list, default=[2,4], help='long_diffusion for lanczos')#[2, 4, 6]
...
def main(args):
    ...
    #     dataset = Planetoid(root='../dataset/data/tmp/',name='Pubmed')
    ...
    datasets = generate_dataset_3class(edge_index, size, save_file, splits=10, probs=args.split_prob, task=args.task,
    ...
    for i in range(10):
        ...
        L = to_edge_dataset_sparse(args.q, edges, args.K, i, size, root=args.data_path + args.dataset, laplacian=True,
                                   norm=args.not_norm, gcn_appr=False)
        ...
        # Ritz1, Q1 = restart_lanczos_approxmiate(args.lanczos_step, L_normal, datapath=storeLanczos)
        Ritz = torch.FloatTensor(Ritz).to(device)
        Qimag = torch.FloatTensor(Q.imag).to(device)
        # Qimg = torch.where(Qimag< 0.01,torch.tensor(0.0).to('cuda'),Qimag)
        Qreal = torch.FloatTensor(Q.real).to(device)
        # Qreal = torch.where(Qreal< 0.01,torch.tensor(0.0).to('cuda'),Qreal)

        X_real = in_out_degree(edges, size).to(device)
        X_real =X_real.to(torch.float32)

        X_img = X_real.to(torch.float32)
        ...
        model = ChebNet_Edge(X_real.size(-1), L_real, L_img, K=args.K, label_dim=args.num_class_link, layer=args.layer,
                             ...
                             num_scale_long=len(args.diffusion), num_eig_vec=args.lanczos_step,
                             long_diff=args.diffusion, R=Ritz, Qimag=Qimag, Qreal=Qreal,edge_index = edge_index).to(device)
        ...
        if args.task == 3:
            ...
            [[val_acc_full_latest, val_acc_latest, val_auc_latest, val_f1_micro_latest, val_f1_macro_latest],
             [test_acc_full_latest, test_acc_latest, test_auc_latest,
              test_f1_micro_latest, test_f1_macro_latest]] = link_prediction_evaluation(out_val, out_test, y_val,
                                                                                        y_test)
            ####################
            # Save testing results
            ####################
            ...
            log_str = log_str.format(val_acc_full_latest=val_acc_full_latest,
                                     ...
                                     test_acc_latest=test_acc_latest,test_auc_latest=test_auc_latest, test_f1_micro_latest=test_f1_micro_latest,
...
if __name__ == "__main__":
    ...
    save_name = args.method_name + 'lr' + str(int(args.lr * 1000)) + 'num_filters' + str(
        int(args.num_filter)) + 'q' + str(int(100 * args.q)) + 'task' + str(int(args.task)) + 'link' + str(
        int(args.num_class_link))
