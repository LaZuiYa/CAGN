...
from layer.MSGNN import MSGNN_node_classification as MSGNN
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='cora_ml/Pubmed', help='data set selection')
    ...
    parser.add_argument('--method_name', type=str, default='MSGNN', help='method name')
...
def main(args):
    ...
    if os.path.isfile(_file_):
        ...
        L = data['L']
        # 数据预处理
        X, label, train_mask, val_mask, test_mask,edge_index = geometric_dataset_sparse(args.q, args.K,
                                                                             root=args.data_path + args.dataset,
                                                                             subset=subset,
                                                                             dataset=load_func, load_only=True,
                                                                             save_pk=False)
    else:
        X, label, train_mask, val_mask, test_mask, L,edge_index  = geometric_dataset_sparse(args.q, args.K,
                                                                                root=args.data_path + args.dataset,
                                                                                subset=subset,
    ...
    for split in range(splits):
        ...
        model = MSGNN(X_real.size(-1),  label_dim=cluster_dim, layer=args.layer,
        ...
        for epoch in range(args.epochs):
            ...
            model.train()
            preds = model(X_real, X_img,edge_index)
