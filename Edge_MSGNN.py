...
from layer.MSGNN import MSGNN_link_prediction as MSGNN
# internal files
...
def parse_args():
    ...
    parser.add_argument('--q', type=float, default=0.0, help='q value for the phase matrix')
...
def main(args):
    ...
    for i in range(10):
        ...
        ########################################
        model = MSGNN(X_real.size(-1),   label_dim= args.num_class_link,layer=args.layer,
                        activation=True, hidden=args.num_filter, dropout=args.dropout).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            model.train()
            out = model(X_real, X_img,edge_index,
                        train_index)
        ...
        if args.task != 2:
            ...
            model.eval()
            out = model(X_real, X_img, edge_index, val_index)[:, :2]
            pred_label = out.max(dim=1)[1]
            val_acc = acc(pred_label, y_val)

            out = model(X_real, X_img, edge_index, test_index)[:, :2]
            pred_label = out.max(dim=1)[1]
            ...
            model.load_state_dict(torch.load(log_path + '/model' + str(i) + '.t7'))
            model.eval()
            out_val =  model(X_real, X_img,edge_index, val_index)
            out_test = model(X_real, X_img,edge_index,test_index)
