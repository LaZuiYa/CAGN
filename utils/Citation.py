...
def citation_datasets(root="./data", alpha=0.1, data_split=10):
    ...
    for split in range(data_split):
        mask = train_test_split(labels, seed=split, val_size=600, test_size=None)
...
def get_train_val_test_split(random_state,
    ...
    if train_examples_per_class is not None:
        ...
        train_indices = random_state.choice(
            remaining_indices, 1800, replace=False)
...
def train_test_split(labels, seed, train_examples_per_class=None, val_examples_per_class=None,
    ...
    train_indices, val_indices, test_indices = get_train_val_test_split(
        random_state, labels, None, val_examples_per_class, test_examples_per_class, train_size,
        val_size, test_size)
...
if __name__ == "__main__":
    graph = load_npz_dataset("../../dataset/data/tmp/cora_ml/")
    adj = graph['A']
    coo = adj.tocoo()
    ...
    #     print(f"The diameter of the graph is {diameter} hops.")
    # else:
    #     # 如果图是不连通的，可以考虑计算每个连通分支的直径，并报告最大值
    #     diameters = [nx.diameter(G.subgraph(component).copy()) for component in nx.connected_components(G)]
    #     max_diameter = max(diameters)
    #     print(f"The graph is not connected. The diameter of the largest connected component is {max_diameter} hops.")
    data = citation_datasets(root="../../dataset/data/tmp/cora_ml/")
    pk.dump(data, open("../../dataset/data/tmp/cora_ml/cora_ml"+'.pk', 'wb'), protocol=pk.HIGHEST_PROTOCOL)
    print(graph['A'])
