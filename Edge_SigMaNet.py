...
from utils.edge_data import link_class_split, in_out_degree, load_signed_real_data_no_negative, \
    load_signed_real_data_also_negative, link_prediction_evaluation
...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='../data/fake/', help='data set folder, for default format see dataset/cora/cora.edges and cora.node_labels')
...
def main(args):
    ...
    #     dataset = Planetoid(root='../dataset/data/tmp/',name='Pubmed')
        ...
        dataset = load_syn(args.data_path + load_func, None)
    ...
    datasets = link_class_split_new(data, save_path= '../data/fake/'+args.dataset + '/' + subset,prob_val=args.split_prob[0], prob_test=args.split_prob[1], splits = 10, task = args.task)
    ...
    for i in range(10):
        ...
        #    size = torch.max(edge_index).item()+1
        X_real = in_out_degree(edge_index, size).to(device)
